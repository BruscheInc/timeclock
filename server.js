...
async function migrate() {
  ...
  await db(`CREATE TABLE IF NOT EXISTS tk_config (
    ...
    week_start INTEGER DEFAULT 1,      -- 0=Sun .. 6=Sat (default Monday)
...
async function getConfig() {
  const r = await db(`SELECT lat,lng,radius_m,week_start,tz,company FROM tk_config WHERE id=1`);
  return r.rows[0] || { lat: null, lng: null, radius_m: 150, week_start: 1, tz: "America/Chicago", company: "Brusche" };
}
...
async function computeTimesheet(fromDate, toDate, onlyEmp) {
  const cfg = await getConfig();
  const tz = cfg.tz || "America/Chicago", weekStart = cfg.week_start ?? 1;
  ...
  for (const [emp, punches] of Object.entries(byEmp)) {
    ...
    // weekly OT
    const weeks = {};
    for (const d of dailyRows) { const wk = weekStartOf(d.day, weekStart); weeks[wk] = (weeks[wk] || 0) + d.hours; }
    let reg = 0, ot = 0;
    for (const h of Object.values(weeks)) { reg += Math.min(h, 40); ot += Math.max(0, h - 40); }
    reg = round2(reg); ot = round2(ot);
    ...
    out.push({ employee: emp, rate, total_hours: round2(reg + ot), reg_hours: reg, ot_hours: ot,
      gross_pay: round2(reg * rate + ot * rate * 1.5), currently: openState, days: dailyRows });
  }
  out.sort((a, b) => a.employee.localeCompare(b.employee));
  return { from: fromDate, to: toDate, week_start: weekStart, tz, employees: out };
}
...
app.post("/api/config", async (req, res) => {
  ...
  try {
    const { lat, lng, radius_m, week_start, tz, company } = req.body || {};
    await db(`UPDATE tk_config SET lat=COALESCE($1,lat), lng=COALESCE($2,lng), radius_m=COALESCE($3,radius_m), week_start=COALESCE($4,week_start), tz=COALESCE($5,tz), company=COALESCE($6,company), updated_at=now() WHERE id=1`,
      [lat ?? null, lng ?? null, radius_m ?? null, week_start ?? null, tz ?? null, company ?? null]);
